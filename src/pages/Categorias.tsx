...
export const Categorias = () => {
  ...
  const carregarCategorias = useCallback(async () => {
    try {
      const res =
        await api.get<ResponseModel<Categoria[]>>("/Categoria/Buscar");
  ...
  const SalvarCategoria = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mensagem de sucesso
      showFlashMessage("Categoria criada com sucesso!", "success");
    } catch (error: unknown) {
      console.error("Erro ao salvar categoria:", error);

      // Mensagem de erro
      showFlashMessage("Erro ao salvar categoria.", "error");
    } finally {
      setCarregando(false);
    }
  };
  ...
  const ApagarCategoria = async (id: number) => {
    ...
    try {
      ...
      // Mensagem de erro
      showFlashMessage("Erro ao excluir categoria.", "error");
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-10">
          ...
          <p className="text-slate-500 font-medium italic">
            Classifique seus ganhos e gastos.
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categorias.length > 0 ? (
            categorias.map((cat) => (
              <div
                key={cat.id}
                className="bg-white p-6 rounded-3xl border border-slate-100 flex items-center justify-between hover:shadow-lg transition-all group"
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center ${cat.finalidade === "Receita" ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
                  >
                    <Tag size={24} />
                  </div>
                  <div>
                    ...
                    <span
                      className={`text-[10px] font-black uppercase ${cat.finalidade === "Receita" ? "text-emerald-500" : "text-rose-400"}`}
                    >
                      {cat.finalidade}
                    </span>
                  </div>
