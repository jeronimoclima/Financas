...
export const Pessoas = () => {
  ...
  const carregarPessoas = useCallback(async () => {
    ...
    try {
      const res = await api.get<ResponseModel<Pessoa[]>>('/pessoa/BuscarPessoas');
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        alert("Erro ao salvar: " + (err.response?.data?.mensagem || "Conexão falhou"));
      }
    } finally {
      setSalvando(false);
    }
    ...
    <div className="p-8 md:p-12 ml-72 min-h-screen bg-[#F8FAFC]">
      <div className="max-w-5xl mx-auto">
        <header className="mb-10 flex justify-between items-center">
          <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight italic">Moradores</h1>
            <p className="text-slate-500 font-medium">Gestão de residentes da casa.</p>
          </div>
          <button onClick={carregarPessoas} className="p-3 text-slate-400 hover:text-blue-600 transition-colors">
            <RefreshCw size={20} className={carregandoLista ? "animate-spin" : ""} />
          </button>
        </header>
        ...
        <section className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white mb-12">
          <form onSubmit={handleSalvar} className="flex flex-col md:flex-row gap-6 items-end">
            <div className="flex-1 w-full">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 mb-2 block">Nome</label>
              <input 
            ...
            <div className="w-full md:w-32">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 mb-2 block">Idade</label>
              <input 
            ...
            className="bg-slate-900 hover:bg-blue-700 text-white  h-14 (56px) px-10 rounded-2xl transition-all shadow-lg flex items-center justify-center disabled:opacity-50">
              ...
              {salvando ? <Loader2 className="animate-spin" size={24} /> : <UserPlus size={22} />}
            </button>
          </form>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {pessoas.map((p) => (
            <div key={p.id} className="bg-white p-6 rounded-3xl border border-slate-100 flex items-center gap-4 hover:border-blue-200 hover:shadow-lg transition-all group relative">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-slate-400">{p.idade} anos</p>
              </div>
              <button onClick={() => handleDeletar(p.id)} className="opacity-0 group-hover:opacity-100 p-2 text-rose-300 hover:text-rose-600 transition-all">
                <Trash2 size={20} />
              </button>
            </div>
          ))}
